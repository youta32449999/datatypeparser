...
class DataTypeCalculatorSpec extends FlatSpec with DiagrammedAssertions {
  ...
  it should "ユーザーデータ型の占有ワード数を算出可能" in {
    ...
    val boolList00_31 = (0 to 15).map(e => TypeAlias("BOOL_ALIAS" + e.toString, "BOOL")).toList
    val intList1_3 = (1 to 3).map(e => TypeAlias("INT_ALIAS" + e.toString, "INT")).toList
    ...
    val typeStructList: Seq[DataType] = Seq(
      ...
      TypeStruct("INT_STRUCT1", intList1_3),
      TypeStruct("INT_STRUCT2", intList1_3 :+ TypeAlias("INT_ALIAS4", "INT")),
      TypeStruct("DINT_STRUCT1", dintList1_3),
      TypeStruct("DINT_STRUCT2", dintList1_3 :+ TypeAlias("DINT_ALIAS", "DINT")),
      TypeStruct("BOOL_ARRAY_STRUCT1", Seq(TypeArray("BOOL_ARRAY1", 0, 31, "BOOL"))),
      TypeStruct("BOOL_ARRAY_STRUCT2", Seq(TypeArray("BOOL_ARRAY2", 0, 7, "BOOL"), TypeArray("BOOL_ARRAY3", 0, 7, "BOOL"), TypeArray("BOOL_ARRAY4", 0, 10, "BOOL")))
      )
